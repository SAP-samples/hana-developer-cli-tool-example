...
import { getCommandExamples, getCommandPresets, getCommandsWithExamples, getCommandsWithPresets, } from '../examples-presets.js';
...
export function handleContentTool(commandName, args) {
    if (commandName === 'examples') {
        const command = args?.command;
        if (!command)
            return errorResponse('Error: command parameter is required');
    ...
    if (commandName === 'recommend') {
        ...
        if (!intent)
            return errorResponse('Error: intent parameter is required. Describe what you want to do in natural language.');
    ...
    if (commandName === 'interpret_result') {
        ...
        if (!command || !result)
            return errorResponse('Error: both command and result parameters are required');
    ...
    if (commandName === 'get_template') {
        ...
        if (!scenario)
            return errorResponse('Error: scenario parameter is required');
