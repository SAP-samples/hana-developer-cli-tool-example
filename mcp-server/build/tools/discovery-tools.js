...
import { CATEGORIES, getCommandsInCategory, searchCommandsByTag, getAllWorkflows, searchWorkflowsByTag, getWorkflowById, } from '../command-metadata.js';
...
export function handleDiscoveryTool(commandName, args) {
    ...
    if (commandName === 'discover_by_category') {
        ...
        if (!category)
            return errorResponse('Error: category parameter is required');
    ...
    if (commandName === 'discover_by_tag') {
        const tag = args?.tag;
        if (!tag)
            return errorResponse('Error: tag parameter is required');
    ...
    if (commandName === 'workflow_by_id') {
        const id = args?.id;
        if (!id)
            return errorResponse('Error: id parameter is required');
