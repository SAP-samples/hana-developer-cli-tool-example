...
function getExploreDatabasePrompt(args) {
...
function getImportDataPrompt(args) {
...
function getTroubleshootConnectionPrompt() {
    return {
        messages: [
            {
                role: 'user',
...
function getValidateDataQualityPrompt(args) {
    const table = args?.table || '[TABLE_NAME]';
    const schema = args?.schema || '[SCHEMA_NAME]';
    const hasTable = table !== '[TABLE_NAME]';
    const hasSchema = schema !== '[SCHEMA_NAME]';
...
function getExportDataPrompt(args) {
    const table = args?.table || '[TABLE_NAME]';
    const schema = args?.schema || '[SCHEMA_NAME]';
