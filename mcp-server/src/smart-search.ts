...
import { COMMAND_METADATA_MAP, getAllWorkflows, searchWorkflowsByTag } from './command-metadata.js';
 ...
 * Search examples
 */
function searchExamples(query: string): SearchResult[] {
  const results: SearchResult[] = [];
 ...
 * Search presets
 */
function searchPresets(query: string): SearchResult[] {
  const results: SearchResult[] = [];
  const commands = Object.keys(COMMAND_METADATA_MAP);
 ...
 */
export function smartSearch(query: string, scope: string = 'all', limit: number = 20): SearchResults {
