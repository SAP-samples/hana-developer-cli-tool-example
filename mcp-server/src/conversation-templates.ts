 ...
 */
export function getConversationTemplate(scenario: string): ConversationTemplate | null {
 ...
 */
export function listConversationTemplates(): { scenario: string; description: string; goal: string }[] {
...
export function searchTemplates(query: string): ConversationTemplate[] {
  ...
  const results: ConversationTemplate[] = [];
  
  for (const template of Object.values(CONVERSATION_TEMPLATES)) {
    if (
      template.scenario.toLowerCase().includes(lowerQuery) ||
