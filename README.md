...
## Description
This tool is intended to be a a developer centric HANA command line tool, particluarly designed to be used when performing local HANA development in non-SAP tooling.  It utilizes the default-env.json that is often used in local development for connectivity to a remote HANA DB (although it can of course be used with a local HANA Express instance as well). There is no intention to replacing the hdbsql tool as a generic SQL console. Instead this tool will focus on simplifing common and complex commands.
