...
describe('inspectTable', function () {
    ...
    it("returns normal output with quiet", function (done) {
        const localTest = base.myTest.bind(this)
        localTest("hana-cli inspectTable -t DUMMY -s SYS --quiet", done)
    })

    it("returns output in SQL format", function (done) {
        const localTest = base.myTest.bind(this)
        localTest("hana-cli inspectTable -t DUMMY -s SYS -o sql --quiet", done)
    })

    it("returns output in CDS format", function (done) {
        const localTest = base.myTest.bind(this)
        localTest("hana-cli inspectTable -t DUMMY -s SYS -o cds --quiet", done)
    })
