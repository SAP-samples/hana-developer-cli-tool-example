...
The `duplicateDetection` command finds duplicate records in HANA tables using various matching strategies. It supports exact matching, fuzzy matching with similarity thresholds, and partial key matching to identify near-duplicates.
...
hana-cli dupdetect --table CUSTOMERS --keyColumns "FIRST_NAME,LAST_NAME"
```
