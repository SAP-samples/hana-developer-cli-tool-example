...
- PostgreSQL and SQLite support on key commands thanks to CDS abstraction
...
- Convert entire project to ECMAScript Modules (ESM) - this is a massive internal change and we apologize in advance if it results in any instability. If you hit any issues please report them, then you can target specifically the last version until they are resolved
...
- Remove @sap/cds-dk as a direct dependency. To use the OpenAPI features in commands like inspectTable, inspectView or cds; you must have @sap/cds-dk globally. But most development environments where this option would be used should already have @sap/cds-dk installed this way. Remove many unneeded dependencies and duplicated modules from hana-cli, most importantly removing the sqlite3 dependency that was never used but caused lots of install issues.
...
- Improved error handling in the Browser Based UI especially during start up scenarios where a default-env.json or equivalent is missing.
...
- massConvert to HDBTABLE or HDBMIGRATIONTABLE without the useCatalogPure option also includes associations now. However when coming from HDBCDS based sources the column names contain dots which are incompatible with CAP CDS. The associations will still have the old column names while the rest of the table definition must replace the dot with underscore or receive CDS Compiler errors. I will leave this functionality in place, although imperfect. If you have associations in your source system, strongly consider using the useCatalogPure option instead.
...
- Upgrade to @sap/cds 5.3.1, @sap/hana-client 2.9.23, @sap/hdbext 7.3.0, and sap-hdbext-promisified 2.202107.1
- Major addition of new browser based UIs for certain complex commands. First focus is on the massConvert command. No new external dependencies. We launch Express web server from the CLI and then open the web browser. You can complete the command with visual options, value help and extended documentation. Command is still executed in the CLI and the output is piped to the web browser using web sockets.
...
- hana-cli version now reports latest available version on npm and prompts users to upgrade if they are outdated
...
- Major internal refactoring to streamline the code and reduce duplication. Reusable base code for basic handling of Yargs and Prompts. Consistent error handling and other general stability improvements
...
- New parameter --debug - for troubleshooting of the hana-cli tool itself. It will produce a LOT of detailed output. Nice if you are curious what queries are being sent to SAP to fulfill a command
- Options in the help are now grouped by Connection Parameters, Troubleshooting, and Options
...
- connect command now prompts for Encrypt setting
...
- Decimal types without decimals generates CDS Error. Issue [#29](https://github.com/SAP-samples/hana-developer-cli-tool-example/issues/29) - thanks to [@ThePlenkov](https://github.com/ThePlenkov)
...
- Issue [#25](https://github.com/SAP-samples/hana-developer-cli-tool-example/issues/25) - Add filename option to massConversion Operation - thanks to [@ThePlenkov](https://github.com/ThePlenkov)
...
- Correct dump if you run *hana-cli* with no command, no defaults to *help* command
